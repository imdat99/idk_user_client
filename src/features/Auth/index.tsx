    ...
    <div className="bg-background flex items-center min-h-svh flex-col">
      <div className="flex-1 items-center flex">
        <div className=":uno: bg-white w-[calc(100vw-1rem)] md:w-[500px] max-w-md rounded-2xl p-8 space-y-6">
          <div className="flex">
            <img className="mx-auto animate-bounce-in duration-500 h-10 w-10" src="/assets/images/logo.svg" alt="logo" />
          </div>
          <Outlet />
        </div>
      </div>
      ...
      <div className="mb-8 text-center text-xs text-gray-500">
        ...
        <div className="mt-2 space-x-4">
          <LanguageBtn />
          <Link to={PolicyPath.privacy} className="hover:text-gray-700">{t("policy.title")}</Link>
          <Link to={contactPath.help} className="hover:text-gray-700">{t("helper.title")}</Link>
        </div>
      </div>
    </div>
  );
