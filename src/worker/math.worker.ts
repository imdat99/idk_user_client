...
const mathOperations = {
  ...
  power: (values: number[]): number => {
    if (values.length !== 2)
      throw new Error('Power operation requires exactly 2 values');
    return Math.pow(values[0], values[1]);
  },

  sqrt: (values: number[]): number => {
    if (values.length !== 1)
      throw new Error('Square root operation requires exactly 1 value');
    if (values[0] < 0)
  ...
  factorial: (values: number[]): number => {
    if (values.length !== 1)
      throw new Error('Factorial operation requires exactly 1 value');
    const n = values[0];
